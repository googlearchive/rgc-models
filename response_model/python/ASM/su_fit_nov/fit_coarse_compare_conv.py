...
def main(argv):
  ...
  for _ in range(n_partitions):
    ...
    tms_validate = perm[np.floor((1 - frac_validate) * perm.shape[0]):
                        perm.shape[0]]
  ...
  if model == 'su':
    ...
    save_filename_partial = os.path.join(FLAGS.save_path_partial,
                                         'Cell_%s_su_nsub_%d_%s_%.3f_frac_'
                                         'train_%.4f_jnt.pkl' %
                                         (ss, nsub, projection_type,
                                          lam_proj, frac_train))
  ...
  if model == 'conv':
    save_filename = os.path.join(FLAGS.save_path,
                                 'Cell_%s_conv_dim_filter_%d_strides_'
                                 '%d_num_filters_%d_frac_train_%.4f_jnt.pkl' %
                                 (ss, dim_filters, strides,
                                  num_filters, frac_train))

    if not gfile.Exists(save_filename):
      print('Fitting started for CONV')

      op = conv_model.convolutional_1layer(stim_use_2d, np.squeeze(resp_use),
                                           partitions[ipartition]['tms_train'],
