...
def main(argv):
  ...
  with tf.Session() as sess:
    ...
    if FLAGS.model_id == 0:
      ...
      loss_with_reg = loss + FLAGS.lam_w*tf.reduce_sum(tf.abs(w)) + FLAGS.lam_a*tf.reduce_sum(tf.abs(a))
      # training steps for a.
      train_step_a = tf.train.AdagradOptimizer(FLAGS.eta_a).minimize(loss, var_list=[a])
