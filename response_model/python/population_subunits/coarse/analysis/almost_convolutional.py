...
# ==============================================================================
"""Model firing of a population using almost convolutional subunits.
...
class AlmostConvolutionalModel(object):
  ...
  def __init__(self, loss_string, stim, resp, short_filename, window=2,
               stride=1, lam_w=0, step_sz=1, n_cells=107):
    ...
    short_filename = ('model=almost_convolutional_window=' +
    ...
    with tf.control_dependencies([gradient_update]):
      b_project_positive = tf.assign(model_vars.bias_cell,
                                     tf.nn.relu(model_vars.bias_cell))

    # Make a combined model update op.
    model_update = tf.group(gradient_update, b_project_positive)
  ...
  def build_variables(self, model_pars):
    ...
    model_variables = collections.namedtuple('model_variables',
                                             ['w_mother', 'w_del', 'a',
                                              'bias_cell', 'bias_su'])
    model_vars = model_variables(w_mother, w_del, a, bias_cell, bias_su)
  ...
  def build_firing_rate(self, model_vars, model_pars, stim):
    ...
    lam = (tf.matmul(tf.reshape(su_act, [-1, dimx*dimy]), a_sfm) +
