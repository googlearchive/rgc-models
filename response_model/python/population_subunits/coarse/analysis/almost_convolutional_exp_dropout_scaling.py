...
class AlmostConvolutionalExpDropoutScale(almost_convolutional.AlmostConvolutionalModel):
  ...
  def __init__(self, loss_string, stim, resp, short_filename, window=2,
    ...
    short_filename = ('model=' + 'almost_convolutional_exp_dropout_scaling' + '_window=' +
  ...
  def build_variables(self, model_pars):
    ...
    w_mother_normalized = tf.Variable(np.array(1 + 0 * np.random.rand(2 * window + 1,
                                             2 * window + 1, 1, 1),
                                    dtype='float32'), name='w_mother_normalized')
    w_mother_scale = tf.Variable(np.array(1, dtype='float32'), name='w_mother_scale')
    w_mother = w_mother_scale * tf.reshape(tf.nn.l2_normalize(tf.reshape(w_mother_normalized, [-1]), dim=0), [2 * window + 1,
                                             2 * window + 1, 1, 1])
    ...
    model_variables = collections.namedtuple("model_variables",
                                             ["w_mother", "w_del",
                                              ...
                                              "w_mother_scale",
                                              "bias_cell_su"])
    model_vars = model_variables(w_mother, w_del, w_mother_normalized,
                                 w_mother_scale, bias_cell_su)
  ...
  def build_firing_rate(self, model_vars, model_pars, stim):
    ...
    select_cells = cells_shuffled[0: n_cells_iter]
