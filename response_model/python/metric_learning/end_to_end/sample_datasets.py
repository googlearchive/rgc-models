...
def get_triplets_nearby_negative(stimulus, responses, delta_t=1, batch_size=100,
                                 stim_history=30, if_continuous=False, rng=None):
  """Get a batch of triplets for form (stimulus, resp_positive, resp_neg).


  Each positive stimulus-response pair has a closeby negative
  chosen at a given time difference,
  ...
  Args:
    ...
    delta_t : Time bin difference between positive and negative responses.
  ...
  if if_continuous:
    print('Do you really want continuous times for positives '
          'when negatives are nearby?')
  ...
  for isample in range(batch_size):
    ...
    resp_batch_neg[isample, :] = resp[itime + delta_t, :]
...
def batch(stimuli, responses, dataset_id, sr_model,
          batch_pos_sz, batch_neg_sz, batch_type='test', delta_t=None,
          frac_cells=1.0, negative_stim=False, if_continuous=False, rng=None):
  ...
  if delta_t is None:
    ...
    if delta_t < 1:
      raise ValueError('Time difference musy be atleast 1 samples')
    if batch_pos_sz != batch_pos_sz:
      raise ValueError('Negatives at time difference 1 can only be generated if'
                       ' positive and negatives of same batch size')
    if negative_stim:
      raise ValueError('RSS triplets not yet supported')
    return_dict = get_triplets_nearby_negative(stimulus,
                                            resp['responses'],
                                            delta_t=delta_t,
                                            batch_size=batch_pos_sz,
                                            stim_history=30,
  ...
  if not negative_stim:
    ...
    if FLAGS.sr_model == 'lin_rank1' or FLAGS.sr_model == 'lin_rank1_blind':
      select_cells_bin = []
      for icell in selected_cell_ids:
        sel_bin = np.zeros(n_cells)
        sel_bin[icell] = 1
        select_cells_bin += [sel_bin]

      select_cells_bin = np.array(select_cells_bin).T.astype(np.float32)
      feed_dict = {sr_model.stim_tf: stim_batch,
                   sr_model.pos_resp: np.expand_dims(resp_batch_pos, 2),
                   sr_model.neg_resp: np.expand_dims(resp_batch_neg, 2),
                   sr_model.select_cells_mat: select_cells_bin}
      ...
      feed_dict = {sr_model.stim_tf: stim_batch,
                   ...
                   sr_model.neg_model.responses_tf:
  ...
  if negative_stim:
    ...
    stim_batch = return_dict['stim_batch']
    resp_batch_pos = return_dict['resp_batch']
    stim_batch_neg = return_dict['stim_batch_neg']
    if FLAGS.sr_model == 'lin_rank1' or FLAGS.sr_model == 'lin_rank1_blind':
      select_cells_bin = []
      for icell in selected_cell_ids:
        sel_bin = np.zeros(n_cells)
      ...
      select_cells_bin = np.array(select_cells_bin).T.astype(np.float32)
      raise ValueError('RSS still not supported for baseline models')
      # TODO(bhaishahster) : support RSS for baseline models.
      '''
      feed_dict = {sr_model.stim_tf: stim_batch,
                   sr_model.pos_resp: np.expand_dims(resp_batch_pos, 2),
                   sr_model.neg_resp: np.expand_dims(resp_batch_neg, 2),
      ...
      feed_dict = {sr_model.stim_tf: stim_batch,
                   ...
                   sr_model.stim_neg_tf: stim_batch_neg
                  }
  ...
  if hasattr(sr_model.anchor_model, 'stim_neg_tf'):
    attrs_list += [sr_model.stim_neg_tf]
