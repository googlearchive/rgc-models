...
# ==============================================================================
r""""Learn a score function of form s(x,y) = (x-y)'A(x-y) with A diagonal PSD.
...
class QuadraticScorePSDDiagonal(quad.QuadraticScore):
  """Score of form x'Ax, with A diagonal PSD"""
  ...
  def _build_graph(self, n_cells, time_window, lr, lam_l1):
    ...
    a_tf = tf.Variable(0.1 * np.ones(self.dim).astype(np.float32))
    self.A_symm = tf.diag(a_tf)
